...
{
  return strcmp(commande, "exit") == 0 || strcmp(commande, "quit") == 0
         || strcmp(commande, "jobs") == 0 || strcmp(commande, "fg") == 0 || strcmp(commande, "bg") == 0 || strcmp(commande, "stop") == 0;
}
...
retoursTraitementCommande executer_commande_interne(char **commande)
{
  ...
  else
  {
    ...
    if(strcmp(commande[0], "jobs") == 0) {
      ...
      //Parcours de la liste des jobs pour afficher leurs caractéristiques (si elle n'est pas vide)
...
retoursTraitementCommande executer_commande_pipe(struct cmdline *l) {
  ...
  //Une commande ne génère qu'un job que si cette dernière ne contient pas que des commandes internes
  int commandeUniquementInterne = 1;

  int numeroJobCommande;
  struct elementListeJobs* jobCommande = NULL;
  ...
  while (l->seq[i] != NULL)
  {
    ...
    //Si la commande est intégrée au Shell, on ne crée pas de fils pour l'exécuter
    if(estCommande_interne(l->seq[i][0]) == 0)
    {
      ...
      else
      {
        ...
        //Création d'un job si ce n'est pas déjà fait
        if(commandeUniquementInterne == 1)
        {
          commandeUniquementInterne = 0;

          numeroJobCommande = AjouterElementListeJobs(listeJobsShell, getChaineCommandeComplete(l), RUNNING);
          jobCommande = GetElementListeJobsByNumero(listeJobsShell, numeroJobCommande);
    ...
    {
      //Si il y a une redirection de l'entrée et que la commande est la première de la série
      if(i == 0 && l->in != NULL) {
        permissions = verification_permissions_fichier(l->in);
        if(permissions == -1) {
          printf("%s: File not found.\n", l->in);
      ...
      //Si il y a une redirection de la sortie et que la commande est la dernière de la série
      //(pas incompatible avec le cas précédent)
      if(l->seq[i + 1] == NULL && l->out != NULL) {
        // Ajout d'une gestion des permissions sur redir sortie pour éviter un "bad file descriptor" (partie 5)
        int i = verification_permissions_fichier(l->out);
